...
app.post('/api/ai/chat', async (req, res) => {
  try {
    ...
    const rawProvider = (provider || '').toString().trim().toLowerCase();
    const rawModel = (model || '').toString().trim().toLowerCase();

    const isAxon =
      rawProvider === 'axon' ||
      rawProvider === 'axon_local' ||
      rawProvider === 'axon-offline-core' ||
      rawProvider === 'local' ||
      rawProvider === 'offline' ||
      rawModel === 'axon-offline-core' ||
      rawModel.includes('axon') ||
      (!rawProvider && !rawModel);

    const systemInstruction = getSystemPrompt(accountLabel, rawProvider || 'axon', projectContext);
    ...
    if (rawProvider === 'chatgpt' || rawProvider === 'openai') {
    ...
    // 4. AXON LOCAL ENGINE (provider === 'axon' or offline fallback or default engine)
    if (isAxon || !['gemini', 'claude', 'chatgpt', 'openai'].includes(rawProvider)) {
      const activeKey = apiKey || process.env.GEMINI_API_KEY;
      if (activeKey) {
        try {
          const ai = new GoogleGenAI({
    ...
    // Default fallback: Always succeed via AXON local engine
    const defaultUserMsg = (messages[messages.length - 1]?.text || '').trim();
    return res.json({
      success: true,
      text: `Notice from AXON Local Core: Processed inquiry for "${defaultUserMsg.slice(0, 80)}" in offline mode.`,
    });
  } catch (error: any) {
