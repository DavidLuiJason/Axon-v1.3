...
export function buildCapabilityRegistry(accounts: AIAccount[]): CapabilityRegistry {
  ...
  // Always register AXON's internal on-device offline reasoning core
  const hasAxonAccount = accounts.some((a) => a.provider === 'axon');
  if (!hasAxonAccount) {
    capabilities.push({
      id: 'cap-axon-local-core',
      name: 'AXON Offline Core',
      provider: 'axon',
      accountId: 'account-axon-core',
      accountLabel: 'AXON Local Core',
      isAvailable: true,
      supportedFeatures: [
        'offline_execution',
        'math_tools',
