...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const switchAccount = async (
    ...
    if (norm === 'axon' || norm.includes('axon local') || norm.includes('offline core') || norm === 'local') {
      setActiveModelId('axon-offline-core');
      const switchText = 'Switched to AXON Local Core. Operating in local on-device offline mode.';
      setMessages((prev) => [
        ...prev,
        {
      ...
      showToast('Switched to AXON Local Core');
  ...
  const addMessage = async (
    ...
    try {
      ...
      console.warn('[AXON Brain] processRequest failed safely, continuing to standard response pipeline:', brainErr);
    }
    ...
    try {
      ...
      if (!response || !data || !response.ok || !data.success) {
        ...
        // If provider is AXON or if unsupported provider error, seamlessly generate offline response
        if (
          activeModel.provider === 'axon' ||
          data?.message?.toLowerCase().includes('unsupported ai provider') ||
          data?.message?.toLowerCase().includes('unsupported provider')
        ) {
